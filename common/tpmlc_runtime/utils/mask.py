...
def mask_seq(src, tokenizer, span_masking, span_geo_prob, span_max_length):
    ...
    for index_set in tokens_index:
        ...
        if span_masking:
            ...
            if prob < 0.8:
                for j in range(span_len):
                    src.replace(src[i + j], str(vocab.get(MASK_TOKEN)))
            elif prob < 0.9:
                for j in range(span_len):
                    while True:
...
def create_index(src, tokenizer, span_masking, span_geo_prob, span_max_length):
    ...
    for (i, token) in enumerate(src):
        if token == vocab.get(CLS_TOKEN) or token == vocab.get(SEP_TOKEN) or token == PAD_ID or token == UNK_TOKEN:
            continue
        if not span_masking:
