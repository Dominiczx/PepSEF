...
class BERTEncoder(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # TransformerEncoderLayer expects `src_key_padding_mask` with True in positions that should be masked (i.e. PADs).
        # Our `attention_mask` convention is 1 for real tokens and 0 for pads, so convert it.
        if attention_mask is None:
            src_key_padding_mask = None
        else:
            # ensure boolean mask where True -> pad
            src_key_padding_mask = (attention_mask == 0)

        for layer in self.layers:
            embeddings = layer(embeddings, src_key_padding_mask=src_key_padding_mask)
        return embeddings
